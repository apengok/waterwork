(function(window,$){
	...
	infoinputList = {
		...
		infoinputTab: function(){
		    ...
		    var columns = [ {
		        ...
		        render: function (data, type, row, meta) {
					...
					//详情按钮
		            result += '<button href="' + detailUrlPath + '" data-target="#commonWin" data-toggle="modal" type="button" class=" editBtn editBtn-info"><i class="fa fa-sun-o"></i>详情</button>&nbsp;';
		            //解除按钮
		...
		showMenu: function(){			
            ...
            $("body").bind("mousedown", infoinputList.onBodyDown);
		},
		onBodyDown:function(event){
			if (!(event.target.id == "menuBtn" || event.target.id == "menuContent" || $(event.target).parents("#menuContent").length>0)) {
				infoinputList.hideMenu();
            }
		},
		hideMenu:function(){
			$("#menuContent").fadeOut("fast");
            $("body").unbind("mousedown", infoinputList.onBodyDown);
		},
			...
			$("#simCardNumber").on("keydown",function(e){
				...
				if(key == "13"){
		        	if($("#simCardNumber").val() == "" || $("#simCardNumber").val() == null){
						setTimeout(function(){$("#simCardNumber").click().focus();},500);
		        	}else{
						setTimeout(function(){$("#terminal").click().focus();},500);
	...
	$(function(){
		...
		//分组显示
		$(".groupZtree").on("click",infoinputList.showMenu);
		//扫码录入显示执行
