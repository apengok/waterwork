(function (window, $) {
    ...
    addConmunityManagement = {
        init: function () {
            var setting = {
                async: {
                    ...
                    dataType: "json",
                    dataFilter: addConmunityManagement.ajaxDataFilter
                },
                view: {
                ...
                callback: {
                    beforeClick: addConmunityManagement.beforeClick,
                    onClick: addConmunityManagement.onClick
                }
            };
            $.fn.zTree.init($("#ztreeDemo"), setting, null);
            // laydate.render({elem: '#installDate', theme: '#6dcff6'});
            // laydate.render({elem: '#procurementDate', theme: '#6dcff6'});
            addConmunityManagement.InitCallback();
        },
        ...
        showMenu: function () {
            ...
            $("body").bind("mousedown", addConmunityManagement.onBodyDown);
        },
        //隐藏菜单
        hideMenu: function () {
            $("#zTreeContent").fadeOut("fast");
            $("body").unbind("mousedown", addConmunityManagement.onBodyDown);
        },
        onBodyDown: function (event) {
            if (!(event.target.id == "menuBtn" || event.target.id == "zTreeContent" || $(
                    event.target).parents("#zTreeContent").length > 0)) {
                addConmunityManagement.hideMenu();
            }
        },
        ...
        InitCallback: function(){
            ...
            var parameter={};
            json_ajax("POST",url,"json",true,parameter, addConmunityManagement.initConcentratorList);
        ...
        ajaxDataFilter: function (treeId, parentNode, responseData) {
            addConmunityManagement.hideErrorMsg();//隐藏错误提示样式
            ...
            if (responseData != null && responseData != "" && responseData != undefined && responseData.length >= 1) {
                ...
                addConmunityManagement.showErrorMsg("您需要先新增一个组织", "zTreeOrganSel");
                return;
            }
        },
        doSubmit: function () {
            if (submissionFlag) {  // 防止重复提交
                ...
                addConmunityManagement.nameValidates();
                if ($("#name").val() != "" && deviceFlag) {
                    if (addConmunityManagement.validates()) {
                        submissionFlag = true;
        ...
        nameValidates: function () {
            ...
            if (name == "") {
                ...
                addConmunityManagement.deviceAjax();
            }
    ...
    $(function () {
        ...
        //显示菜单
        $("#zTreeOrganSel").bind("click", addConmunityManagement.showMenu);
        //提交
        $("#doSubmit").bind("click", addConmunityManagement.doSubmit);
    })
})(window, $)
